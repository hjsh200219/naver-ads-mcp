...
helloMAX 주간/데일리 광고 리포트 자동화 + 광고주별 Live Artifact 대시보드를 `naver-ads-mcp`에 확장 형태로 구현하는 합의 플랜이 v1.1 LOCKED 상태로 산출됨. 산출물 위치: `docs/exec-plans/active/weekly-report-automation-plan.md` + 4 review + 1 codex review.
...
**Phase 1 GO 게이트** (모두 충족해야 진입):

1. ZBROS 월 USD 결재 라인 서면 합의
2. Phase 0 PoC 4건: artifact↔MCP 콜백 capability 실측 / Claude Max seat 사용량 한도 측정 / 택스아이 누락 고지 정책 결정 / PII 최소화 prompt 설계
