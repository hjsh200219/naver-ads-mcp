...
#### Option A — Live Artifact (preview-first) + MCP 확장 ★ 권장

`naver-ads-mcp`에 **tool 2개 + resource 2개** 추가 (확장 후 5 tools + 4 resources).

| 종류     | 이름                                                 | 사유                |
| -------- | ---------------------------------------------------- | ------------------- |
| Tool     | `prepare_weekly_dashboard(client, week, revisions?)` | compute action      |
| Tool     | `send_report_email(payload_hash, confirm)`           | side-effect-bearing |
| Resource | `naver-ads://client-mappings`                        | read-only metadata  |
| Resource | `naver-ads://history/{client}`                       | read-only audit log |

흐름: AE → `prepare_weekly_dashboard` → MCP 가 KPI 사전계산 + Anthropic → JSON+payload_hash+artifact HTML → Claude artifact 렌더 → AE 자연어 수정 (default) 또는 inline 편집 (opt-in) → "발송해" → `send_report_email(hash, confirm=true)` → MCP가 hash 검증 + lock 획득 + SES/EML 발송 + history JSONL append.
...
## Architecture (Option A v0.3)

```
┌─────────────────────────────────────────────────────────┐
│ AE (Claude Desktop)                                     │
│   "비셰프 4월 4주차 리포트" → "ROAS 톤 부드럽게" → "발송해" │
└──────────────────┬──────────────────────────────────────┘
                   │ MCP tool/resource
                   ▼
┌─────────────────────────────────────────────────────────┐
...
│   • src/runtime/email-send.ts       [NEW] SES/EML       │
...
| `src/runtime/email-send.ts`       | L1    | nodemailer/aws-sdk, L2 dashboard            | —               |
...
| 6   | 1-2-④ 광고주 이메일    | 中                  | 3     | runtime/email-send                         |
...
- [ ] **PoC 2**: Email 채널 결정
  - SES + hellomax.co.kr 도메인 인증(DKIM/SPF) 비용/일정
...
- [ ] `src/runtime/email-send.ts` — Phase 0 결정 채널 (SES default + EML fallback)
- [ ] `src/runtime/lock.ts` — `(client, week)` 단위 file lock (proper-lockfile)
- [ ] `src/runtime/history.ts` — atomic append (write temp + rename) + JSONL 스키마
...
- [ ] MCP tool `send_report_email(payload_hash, confirm)` (hash 검증 + lock + idempotency)
- [ ] 발송 후 5분 status 재확인 + 실패 시 Slack 알림
- [ ] **Concurrency 테스트**: 같은 (client, week) 동시 send_report_email 2건 → 1건만 통과, 1건 lock 거절. 2건 모두 다른 hash로 발송 시도 시 두 번째도 거절(client+week 단위 발송 1회 정책)

**완료 기준**: 6 광고주 중 2개에 SES 테스트 발송 (qa+ inbox) + history JSONL atomic append 검증 + concurrency 테스트 통과 + 신규 14+ tests.
...
- [ ] **AE 교육 워크숍 1회 (2시간)**: 자연어 편집 cheat sheet 5개 예시 + artifact 사용법 + 발송 절차
...
- [ ] **머신 분실/AE 교체 runbook** (`docs/SECURITY.md` 보강 + 본 plan 양방향 cross-link)
  - 자격증명 회전 (Naver / Anthropic / SES) — 각 서비스별 콘솔 URL + 절차
  - history JSONL 백업 정책: **주 1회 1Password Secure Note** (외부 안전 저장소)
...
**완료 기준**: AE 만족도 평균 4/5 (표본 ≥ 2명, 익명) + 광고주 발송 6/6 + 0 hallucination 인스턴스 + 매뉴얼 검증 통과 + runbook PR merge.

### (Out of scope, v2) Phase 5 — 신규 광고주 온보딩

총 8.5주 (Phase 합산 9주, 아래 병렬 가정 적용 시 8.5주).

### [NEW v0.4] 일정 병렬 가정 (Gantt 1줄)
...
P2          ████████████████
P3                          ███████
P3.5                        ███████   (Phase 3과 병렬: runtime 모듈 공유, 데일리 임계값 정의는 P3 종료 후 시작)
P4                                  ███████████████  (P3·P3.5 ship 후 시작)
```

**병렬 구간 명시**:

- Phase 3 (이메일 발송) + Phase 3.5 (데일리) 일부 병렬: `runtime/lock.ts`·`runtime/history.ts`·`runtime/alert.ts` 모듈을 P3에서 1차 구현, P3.5는 그 위에 데일리 분석기·임계값 + Slack 알림 라우팅만 얹음. P3 1주 + P3.5 0.5주 슬라이스 병렬로 합산 1.5주에 처리 (단순 직렬은 2주).

**주의**: Phase 0 + Phase 1 + Phase 2는 직렬 강제 (선행 산출물 의존성). 병렬 가정 실패 시 9주로 fallback — Phase 1 GO 시점에 `docs/exec-plans/active/timeline-tracker.md`에 실제 일정 기록 필수.
...
| 3     | SES 신뢰도 손상 → `MCP_EMAIL_CHANNEL=eml` toggle (EML export only)                      |
...
- **워크숍 1회 (2시간)**:
  ...
  - **[NEW v0.4] force-resend 절차**: 발송 후 광고주에게 hallucination·오타 발견 시 (a) 광고주에게 정정 안내 메일 별도 발송, (b) `send_report_email --payload_hash=NEW --confirm=true --force-resend=true` 호출, (c) history JSONL `status=corrected` 표기. 이 절차는 ZBROS 내부 보안 1인 사전 승인 필요.
...
| 9   | Email 채널 + 도메인 인증                                                                                         | `dig`/`dkim` 검증 명령 (`docs/SECURITY.md` 절차)                                                     |
...
| 7   | `send_report_email --payload_hash=... --confirm=true` (qa inbox)               | Gmail/SES 수신                    |
...
| 12  | dig/dkim/spf hellomax.co.kr                                                    | DNS 검증                          |
...
3. Data integrity — 0 hallucination (1차 ship 95%, Phase 4 99%); 광고주 신뢰 보호
...
| 3      | email 채널 default                                                | SES default + EML fallback                            | **결정** (Phase 0 PoC가 뒤집을 여지: 도메인 인증 비용 초과 시 EML primary로 변경) |
...
## Open Questions (Phase 1 GO 전 답변 필요)
