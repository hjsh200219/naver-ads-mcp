# helloMAX Weekly Report Automation — Plan v1.5 (FINAL)

> 상태: **FINAL v1.5** (v1.4 + 사용자 결정: 이메일 발송은 **별도 MCP**(예: Gmail MCP)가 담당. naver-ads-mcp는 email payload 생성까지만)
...
> **v1.5 변경점**: 이메일 전송은 naver-ads-mcp 책임 아님 — **별도 MCP**(Gmail MCP 등)에 위임. naver-ads-mcp는 `prepare_weekly_dashboard`가 artifact + email_payload(To/Cc/Subject/HTML body/첨부 path)만 반환. AE가 Claude Desktop에서 다른 MCP로 발송 호출. SMTP 모듈·nodemailer 의존성 제거. send_report_email tool 제거.
...
#### Option A — Live Artifact (preview only) + 별도 Email MCP 위임 ★ 채택 (v1.5)

`naver-ads-mcp`에 **tool 1개 + resource 2개** 추가 (확장 후 4 tools + 4 resources).

| 종류     | 이름                                                 | 사유                                                            |
| -------- | ---------------------------------------------------- | --------------------------------------------------------------- |
| Tool     | `prepare_weekly_dashboard(client, week, revisions?)` | artifact HTML + **email_payload**(To/Cc/Subject/HTML/첨부) 생성 |
| Resource | `naver-ads://client-mappings`                        | read-only metadata (수신자 정보 포함)                           |
| Resource | `naver-ads://history/{client}`                       | read-only prepare 이력 (send 이력은 외부 MCP)                   |

흐름 (v1.5): AE → `prepare_weekly_dashboard` → MCP가 KPI 사전계산 + Anthropic → JSON + payload_hash + **artifact HTML + email_payload** → Claude **artifact preview only 렌더** → AE 자연어 수정(default) → "발송해" → Claude가 **별도 MCP**(예: Gmail MCP `create_draft` 또는 그에 준하는 send tool)에 email_payload 전달 → 발송. naver-ads-mcp는 prepare 이력만 history JSONL에 append.

### v1.5 핵심 (책임 분리)

- ✅ naver-ads-mcp 책임: **Naver 데이터 + AI 분석 + artifact preview + email_payload 생성**까지
- ✅ 발송 책임: **별도 MCP**(Gmail MCP 등)가 담당. naver-ads-mcp는 발송 자체에 관여 안 함
- ✅ artifact는 preview only (v1.3 유지)
- **제거된 것** (v1.4 → v1.5):
  - ❌ `send_report_email` tool
  - ❌ `src/runtime/email-send.ts` (nodemailer 호출)
  - ❌ accounts.json `smtp` 자격증명 키
  - ❌ nodemailer 의존성
  - ❌ SES/도메인 인증/DKIM/SPF (이미 v1.4에서 미도입)
- email_payload 형식: `{ to: string, cc: string[], subject: string, html_body: string, attachment_path: string }` — 다른 MCP가 그대로 소비 가능한 표준 구조
- naver-ads-mcp `history/{client}` JSONL은 prepare 이벤트만 기록 (`{week, payload_hash, prepared_at}`). send 이벤트는 외부 MCP의 sent items가 source of truth
...
└──────────────┬─────────────────────────┬─────────────────────┘
               ...
               ▼                         ▼
┌──────────────────────────┐   ┌──────────────────────────────┐
│ naver-ads-mcp (확장)      │   │ Gmail MCP (외부)              │
│  Tools                   │   │   • create_draft / send       │
│   • prepare_weekly_       │   │   • list_drafts               │
│     dashboard      [NEW] │   │   • search_threads            │
│   • validate_creds,       │   │                               │
│     fetch_raw_data,       │   │ AE가 email_payload 받아       │
│     generate_report       │   │ Gmail MCP에 전달 → 발송        │
│  Resources                │   └──────────────────────────────┘
│   • naver-ads://report-   │
│     types, accounts       │
│   • naver-ads://client-   │
...
│   • dashboard/artifact-html                [NEW]
│   • email/payload-builder (v1.5)           [NEW] To/Cc/Subj/HTML 표준 JSON
│   ❌ email/builder.ts MIME (v1.5 제거 — 외부 MCP가 처리)
│                            │
│  L3 API                  │
│   • anthropic.ts         [NEW]
│   ❌ nodemailer / AWS SES — 의존성 0
│                            │
│  L4 config               │
│   • credentials.ts (Anthropic 추가, enumerable:false)
│   • client-mappings.json [NEW] (수신자·참조 포함)
...
| `src/email/payload-builder.ts` (v1.5) | L2    | L2 dashboard (HTML 본문), L5 types          | L1, L3, L4      |
...
| 6   | 1-2-④ 광고주 이메일    | 中                  | 3     | email/payload-builder (외부 Email MCP가 발송) |
...
- [ ] **PoC 2 (v1.5)**: 외부 Email MCP 연동 검증
  - email_payload `{ to, cc, subject, html_body, attachment_path }` 표준 JSON 정의
  - Gmail MCP `create_draft` 등에 payload 전달 → 정상 draft 생성·발송 (qa inbox)
  - HTML 본문 + 첨부 정상 수신, UTF-8 보존, from = AE 본인 Gmail 계정
  - 어떤 외부 MCP를 default로 할지 결정 (Gmail MCP / 다른 메일 MCP 등)
...
### Phase 3 — Email payload + 인프라 (0.5주, v1.5 단순화)

- [ ] `src/email/payload-builder.ts` (v1.5) — artifact JSON → 표준 email_payload `{ to, cc[], subject, html_body, attachment_path }` 생성 (외부 MCP 소비용). MIME 인코딩·실제 발송 없음
- [ ] `src/runtime/lock.ts` — `(client, week)` 단위 file lock (proper-lockfile)
- [ ] `src/runtime/history.ts` — atomic append (write temp + rename) + JSONL 스키마. **prepare 이벤트만 기록** (`{week, payload_hash, prepared_at, status=prepared}`). send 이벤트는 외부 MCP의 sent items가 source of truth
...
- [ ] `prepare_weekly_dashboard` 반환값에 `email_payload` 필드 추가
- [ ] **Concurrency 테스트**: 같은 (client, week) 동시 prepare 2건 → 1건만 통과, 1건 lock 거절
- [ ] AE 발송 가이드 문서: Claude Desktop에서 prepare 후 "이 payload로 Gmail로 보내줘" → Gmail MCP `create_draft`/send 호출 흐름

**완료 기준**: 6 광고주 fixture로 email_payload 정상 생성 + Gmail MCP가 그 payload로 draft 생성 성공 (수동 검증) + history prepare 이벤트 atomic append + concurrency 테스트 통과 + 신규 10+ tests.
...
총 **약 8주 (1인 개발, v1.5)**. Phase 3 = email_payload 빌더 + 인프라 = 0.5주로 단축.
...
P1      █████████          (1.5주)
P2          ████████████████  (2.5주)
P3                          ████  (0.5주, payload-builder + 인프라)
P3.5                            ████████  (1주, 데일리)
P4                                      ████████████████  (2주)
```

**v1.5 핵심 (책임 분리, 추가 개발 0)**:

- 발송은 외부 MCP가 담당 → naver-ads-mcp는 email_payload 표준 JSON만 반환
- nodemailer / SMTP / SES / 도메인 인증 / MIME builder 모두 의존성 0
- `email/payload-builder.ts` ~80 LOC 수준 (단순 객체 매핑 + 첨부 path 결합)
- artifact는 preview only (v1.3 유지)
...
- **워크숍 1회 (2시간)**:
  ...
  - **[v1.5] 정정 발송 절차**: 발송 후 hallucination·오타 발견 시 (a) MCP에서 새 `prepare_weekly_dashboard(..., revisions=..., correction=true)` 호출 → 새 email_payload (본문 시작에 "정정 안내" 문구 자동 삽입), (b) AE가 외부 Email MCP에 새 payload 전달해 발송, (c) history JSONL `status=corrected_prepared` entry append. 실제 발송 이력은 외부 Email MCP가 source of truth.
...
| 9   | email_payload 표준 JSON 정상 생성 + 외부 Email MCP가 그 payload로 draft/send 성공 (qa inbox 수신)                | payload schema 자동 검증 + Gmail MCP 수동 검증                                                       |
...
| 7   | prepare 결과의 email_payload를 Gmail MCP `create_draft`에 전달 → draft 생성 → qa inbox 발송     | 수동 검증 (외부 MCP 흐름)         |
...
**Decision**: Option A — Live Artifact **preview only** + **email_payload 생성 후 외부 Email MCP에 위임** + `naver-ads-mcp` 확장 (tool +1 / resource +2 → 4T/4R). **`prepare_weekly_dashboard`만** (v1.5: send tool 제거). naver-ads-mcp는 데이터·AI·preview·payload 생성, 발송은 Gmail MCP 등 외부 MCP 책임.
...
- ⊕ 인프라 0 (stdio MCP + file lock + JSONL audit). nodemailer/SES/SMTP/MIME 모두 의존성 0
...
- ⊖ 발송 자동화는 외부 MCP 의존 → Gmail MCP outage 시 발송 흐름 차단 (단 prepare 결과는 보존되므로 재발송 가능)
- ⊖ 발송 이력 통합 audit 없음 (naver-ads-mcp는 prepare만, send는 외부 Email MCP가 별도 추적)
...
| 3      | email 채널 default                                                | **외부 Email MCP** (Gmail MCP 등) — naver-ads-mcp는 email_payload만 생성 (v1.5) | **결정** (SMTP·SES·nodemailer 모두 미도입) |
